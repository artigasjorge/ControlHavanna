...
def render_combo_quick_lookup(combos_list: list[Combo], combo_articles: list[Articulo]) -> None:
    with st.container(border=True):
        ...
        selected_combo = st.selectbox(
            "Combo",
            combos_list,
            format_func=lambda combo: f"{combo.codigo} - {combo.descripcion}",
        ...
        article_names = [
            ...
            if article.combo_codigo == selected_combo.codigo
        ]

        if article_names:
            st.write(";   ".join(article_names))
        else:
            st.write("Sin articulos vinculados.")
    ...
    try:
        ...
        combo_articles = list(
            db.scalars(
                ...
                .order_by(Articulo.combo_codigo, Articulo.nombre)
            ).all()
        )
    finally:
        db.close()
...
def reportes_cerrados() -> None:
    ...
    for reporte in reportes:
        ...
        with row[3]:
            if st.button("Informe Ventas por turno", key=f"informe_ventas_{reporte.id}"):
                informe_ventas_dialog(reporte.id)
        ...
        with row[5]:
            if st.button("Informe check vs sistema", key=f"check_sistema_{reporte.id}"):
                st.info("Informe check vs sistema sera desarrollado en una etapa posterior.")
