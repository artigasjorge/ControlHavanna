...
def informe_ventas_dialog(reporte_id: int) -> None:
    ...
    st.dataframe(
        [
            {
                "Fecha": reporte.fecha_creacion.strftime("%Y-%m-%d %H:%M"),
...
def ventas() -> None:
    ...
    if st.session_state.confirm:
        ...
        with centro:
            ...
            with s:
                if st.button("SI"):
                    save_reporte_ventas(local, turno, hi, hc, enc, cats, combos_list)
                    reset_ventas_form()
                    st.rerun()
...
def reportes_cerrados() -> None:
    ...
    for reporte in reportes:
        ...
        row[0].write(reporte.fecha_creacion.strftime("%Y-%m-%d"))
